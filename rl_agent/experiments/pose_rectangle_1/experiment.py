...
def create_environment():
    env = Box2DPushingEnv(smoothDraw=False, reward=RewardFunctions.PROJECTION, max_steps=200) # Was ran with only rectangle
    env = GymWrapper(env)
...
def run(exp_num):
    ...
    # Save model
    # savers.save_to_path('learner_checkpoint', agent._learner.save())
